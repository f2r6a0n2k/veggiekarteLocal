...
def get_osm_data(searchArea):
    ...
    #overpass_query += searchArea #"nwr(area.searchArea)['vegan'~'yes|only|friendly'];"
    ...
    while (server < len(SERVERS)) and (result is None):
        ...
        # Overpass request
        print("Send query to server: ", overpass_server + overpass_query)
        ...
        if osm_request.status == 200:
            ...
            #print(osm_request.data)
            OVERPASS_FILE.touch()
            OVERPASS_FILE.write_bytes(osm_request.data)

            result = osm_request.data
            #json.loads(osm_request.data.decode("utf-8"))
...
def main():
    ...
    # Get data
    if len(sys.argv) < 2:
        osm_data = get_osm_data("nwr(area.searchArea)['vegan'~'yes|only|friendly'];")
        # osm_data += get_osm_data("nwr(area.searchArea)['diet:vegan'~'yes|only|limited'];")
        # Store the raw output in a file (for any later use)

        osm_data = json.loads(osm_data.decode("utf-8"))
